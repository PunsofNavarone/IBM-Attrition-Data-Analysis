...
```
### Randomly generating the missing values
```{r}
set.seed(50)
df[sample(1:nrow(df), floor(runif(1, min=10, max=60))), "Age"] <- NA
df[sample(1:nrow(df), floor(runif(1, min=10, max=60))), "Department"] <- NA
df[sample(1:nrow(df), floor(runif(1, min=10, max=60))), "HourlyRate"] <- NA
df[sample(1:nrow(df), floor(runif(1, min=10, max=60))), "MonthlyIncome"] <- NA
df[sample(1:nrow(df), floor(runif(1, min=10, max=60))), "DistanceFromHome"] <- NA
...
###Using mice package for missing data imputation
```{r}
...
meth[c("DistanceFromHome")]="norm" 
meth[c("Department")]="polyreg" 
meth[c("MonthlyIncome")]="norm" 
meth[c("Age")]="norm"
meth[c("HourlyRate")]="norm"
```

```{r}
set.seed(103)
df = mice(df, method=meth, predictorMatrix=predM, m=1)
```
...
### creating N/A for the perpose of MNAR 
```{r}
#Removing the 15% of the data who has the jobsatisfaction as 1
df$JobSatisfaction = as.factor(df$JobSatisfaction)
df <- delete_MNAR_censoring(df, 0.15,"JobSatisfaction")
```

```{r}
chisq.test(df$JobSatisfaction,df$EnvironmentSatisfaction)

chisq.test(df$JobSatisfaction,df$MonthlyIncome)

chisq.test(df$JobSatisfaction,df$JobInvolvement)

chisq.test(df$JobSatisfaction,df$YearsWithCurrManager)

chisq.test(df$JobSatisfaction,df$YearsInCurrentRole)

chisq.test(df$JobSatisfaction,df$DistanceFromHome)

chisq.test(df$JobSatisfaction,df$OverTime)


```
We can conclude from the above hypothesis test that their is no relationship between Job satisfaction and other independent variables. Therefore we are going to use the mice package to impute the data.

```{r}
meth[c("JobSatisfaction")]="polyreg"

df = mice(df, method=meth, predictorMatrix=predM, m=5)
...
#Data analysis by Visualization of Attrition
```{r}
df %>%
        group_by(Attrition) %>%
        tally() %>%
...
```
The people who are traveling frequently are more probable to leave the organization
...
```
there is no impact of gender on  attrition
...
```
Clearly the people who has low income are leaving the organization
...
Department_HR:Joberole_HR(.92)
Department_HR:Edfeild_HR(0.66)
...
#Removing the feature which has highest P-value
X_train <- subset(X_train, select = - c(JobRole_Laboratory_Technician))
X_test <- subset(X_test, select = - c(JobRole_Laboratory_Technician))

model_13 <- glm( Attrition_Yes~., data = X_train, family=binomial)
#Removing the feature which has highest P-value
X_train <- subset(X_train, select = - c(DistanceFromHome))
X_test <- subset(X_test, select = - c(DistanceFromHome))
...
model_21<- glm( Attrition_Yes~., data = X_train, family=binomial)
summary(model_21)
...
predictions <- predict(model_21, X_test,type="response")
dim(X_test)
